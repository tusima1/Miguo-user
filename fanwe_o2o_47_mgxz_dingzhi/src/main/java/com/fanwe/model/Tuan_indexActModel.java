...
public class Tuan_indexActModel extends BaseActModel implements MapSearchModelSupplier
{
	...
	// mgxz
	private List<GoodsGroupModel> item;
	...
	public List<MapSearchBaseModel> getListMapSearchModel()
	{
		...
		if (!SDCollectionUtil.isEmpty(item))
		{
			...
			for (GoodsGroupModel model : item)
			{
				List<GoodsModel> listGoods = model.getDeal_data();
				if (!SDCollectionUtil.isEmpty(listGoods))
				{
					for (GoodsModel goodsModel : listGoods)
					{
						listMapModel.add(goodsModel.createMapSearchModel());
					}
				}
			}
		}
	...
	public List<GoodsGroupModel> getItem()
	{
		return item;
	}

	public void setItem(List<GoodsGroupModel> item)
	{
		this.item = item;
	}
