 ...
 * 消息列表的Adapter
 */
public class LiveChatMsgListAdapter extends BaseAdapter implements AbsListView.OnScrollListener {

    private static String TAG = LiveChatMsgListAdapter.class.getSimpleName();
    private static final int ITEMCOUNT = 7;
    private List<LiveChatEntity> listMessage = null;
    ...
    private ArrayList<LiveChatEntity> myArray = new ArrayList<LiveChatEntity>();
    ...
    public LiveChatMsgListAdapter(Context context, ListView listview, List<LiveChatEntity> objects) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        LiveChatEntity item = listMessage.get(position);
...
//        holder.sendContext.fixViewWidth(mListView.getWidth());
     ...
     * @param item
     */
    private void continueAnimator(View itemView, int position, final LiveChatEntity item) {
        ...
        if (animatorIdx < MAXANIMATORCOUNT) {
            ...
            if (position < mAnimatorInfoList.size()) {
                ...
                if (during < 0) {   // 剩余时长小于0直接设置透明度为0并返回
                    ...
                    SxbLog.v(TAG, "continueAnimator->already end animator:" + position + "/" + item.getContent() + "-" + during);
                    return;
                }
            }
     ...
     * @param item     列表数据
     */
    private void playViewAnimator(View itemView, int position, final LiveChatEntity item) {
